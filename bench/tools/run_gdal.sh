...
# BLOCKSIZE takes one value per array dimension: a (t, y, x) input gets 1 on the leading axis
NDIM=$(python3 -c "import json,sys; print(len(json.load(open('$IN/data/zarr.json'))['shape']))" 2>/dev/null || echo 2)
BLOCK="$BS,$BS"; [ "$NDIM" = 3 ] && BLOCK="1,$BS,$BS"
CID=gdalzarr_$$
t0=$(date +%s.%N)
docker run -d --name $CID -u "$(id -u):$(id -g)" -e GDAL_NUM_THREADS=8 -e GDAL_CACHEMAX=4096 \
  -v "$IN:/in:ro" -v "$OUTDIR:/out" $IMG bash -c "
    gdal_translate -q -of ZARR -co FORMAT=ZARR_V3 -co BLOCKSIZE=$BLOCK -co COMPRESS=ZSTD 'ZARR:\"/in\":/data' /out/$NAME &&
...
printf '{"impl": "gdal-3.13.3", "ok": %s, "error": %s, "wall_s": %.2f, "objects": %d, "peak_rss_mb": %.1f}\n' \
