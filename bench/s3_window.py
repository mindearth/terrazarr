...
per-task timings from the task stream.
    ...
    source bench/s3env.sh; .venv/bin/python bench/s3_window.py 32768 out.zarr [y0 x0] [s3://store]
...
if __name__ == "__main__":
    ...
    with get_task_stream(client=client) as ts:
        ...
        create_geozarr_dataset(xr.DataTree(ds), groups=["/"], output_path=out, spatial_chunk=4096, min_dimension=256, tile_width=256,
                               max_retries=1, enable_sharding=True, method="mean", nodata_value=0, compressor=make_compressor("zstd", 3))
        sys.stdout = so
    ...
    print(f"window {n}²: shards(all bands)={shards} tasks={len(ts.data)} wall={wall:.1f}s worker compute={tot:.1f}s ({tot/shards*1000:.0f} ms per shard) objects={sum(len(f) for _,_,f in os.walk(out))} l0 shards stored={l0.nchunks_initialized}")
