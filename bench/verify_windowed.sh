...
for mode in "w8 8" "t8 1"; do set -- $mode
  ...
  /usr/bin/time -f 'time: wall=%e cpu=%P maxrss=%MKB' .venv/bin/python bench/run_one.py --impl optimized --input $IN --output $D/win_windowed_$1.zarr --chunk-size 4096 --tile-width 256 --method mean --nodata 0 --sharding --threads 8 --workers $2 --quiet 2> $OUT/windowed_win32k_$1.stderr | tail -1 | tee $OUT/windowed_win32k_$1.json
...
/usr/bin/time -f 'time: wall=%e cpu=%P maxrss=%MKB' .venv/bin/python bench/run_one.py --impl optimized --input $D/input/WSF3Dv3_Italy_full.zarr --output $D/full_windowed_w8.zarr --chunk-size 4096 --tile-width 256 --method mean --nodata 0 --sharding --threads 8 --workers 8 --quiet 2> $OUT/windowed_full_w8.stderr | tail -1 | tee $OUT/windowed_full_w8.json
